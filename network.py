...
class MM:
    ...
    def ensemble(self, trans_tmp, ae_tmp, mmin_tmp):
        ...
        wei_tr = tf.nn.softmax(tf.reshape(tf.concat([t_va, t_vt, t_at], 0), [3, -1] ), 0)


        a_va = tf.reduce_max(ae_va, -1)
        a_vt = tf.reduce_max(ae_vt, -1)
        a_at = tf.reduce_max(ae_at, -1)
        wei_ae = tf.nn.softmax(tf.reshape(tf.concat([a_va, a_vt, a_at], 0), [3,-1]), 0)

        m_v = tf.reduce_max(mm_va, -1)
        m_a = tf.reduce_max(mm_vt, -1)
        m_t = tf.reduce_max(mm_at, -1)
        wei_mm = tf.nn.softmax(tf.reshape(tf.concat([m_v, m_a, m_t], 0), [3,-1]), 0)
        ...
        for i in range(self.config.batch_size):
            if (self.flag[i] == 0 and tf.argmax(res_vat[i]) != tf.argmax(res_at[i])) or (self.flag[i] == 1 and tf.argmax(res_vat[i]) != tf.argmax(res_vt[i])) or (self.flag[i] == 2 and tf.argmax(res_vat[i]) != tf.argmax(res_va[i])):
                tmp_t = tf.multiply(trans_tmp['va'][i], wei_tr[0][i]) + tf.multiply(trans_tmp['vt'][i], wei_tr[1][i]) + tf.multiply(trans_tmp['at'][i], wei_tr[2][i])
                tmp_ae = tf.multiply(ae_tmp['va'][i], wei_ae[0][i]) + tf.multiply(ae_tmp['vt'][i], wei_ae[1][i]) + tf.multiply(ae_tmp['at'][i], wei_ae[2][i])
                tmp_mm = tf.multiply(mmin_tmp['va'][i], wei_mm[0][i]) + tf.multiply(mmin_tmp['vt'][i], wei_mm[1][i]) + tf.multiply(mmin_tmp['at'][i], 
wei_mm[2][i])
                #tmp_ = (tmp_t + tmp_ae + tmp_mm)/3
                concat_all = tf.nn.tanh(tf.transpose(tf.concat([tmp_t, tmp_ae, tmp_mm], 0)))
