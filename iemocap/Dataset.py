...
class Dataset:
    def __init__(self):
        self.config = Config()
        self.iter_num = 0
        self.label_set = ['Negative', 'Positive']
...
#'neu', 'xxx', 'fru', 'ang', 'sad', 'hap', 'exc', 'sur', 'oth', 'fea', 'dis'
    def setdata(self, train_size):
        ...
        valdata = {'ID':[], 'V':[], 'A':[], 'T':[], 'L':[], 'F':[]}
        testdata = {'ID':[], 'V':[], 'A':[], 'T':[], 'L':[], 'F':[]}
        temp_order = list(range(len(self.label)))
        np.random.shuffle(temp_order)
        for i in range(len(self.keys)):
            cur_id = self.keys[i]
            if self.label[cur_id] in ['hap', 'exc']:
                tmp_label = 'Positive'
            elif self.label[cur_id] in ['neu', 'xxx', 'oth']:
                tmp_label = 'Neutral'
            else:
                tmp_label = 'Negative'
            if tmp_label == 'Positive' or tmp_label == 'Negative':
              if i < train_size-2000:
              ...
              elif i>=train_size-2000 and i< train_size:
                  valdata['ID'].append(cur_id)
                  valdata['V'].append(self.padding(self.visual[cur_id], self.max_visual_len))
                  valdata['A'].append(self.padding(self.audio[cur_id], self.max_audio_len))
                  valdata['T'].append(self.padding(self.text[cur_id], self.max_text_len))
                  valdata['L'].append(self.label_set.index(tmp_label))
                  valdata['F'].append(self.flag[cur_id])
              else:
...
if __name__ == '__main__':
    ...
    traindata, valdata, testdata = data.setdata(8000)
#    print (traindata['A'][:2])

    pickle.dump(traindata, open('./train.pkl', 'wb'))
    pickle.dump(valdata, open('./val.pkl', 'wb'))
