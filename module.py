...
def get_onehot(pro):
    a = np.argmax(pro, -1)
    b = np.zeros((np.shape(pro)[0], np.shape(pro)[1]))
    b[np.arange(int(np.shape(pro)[0])), a] = 1.0
    return b

def cross_ent(y, y_pre):
    y = tf.cast(y, tf.float64)
    y_pre = tf.cast(y_pre, tf.float64)
    loss = -tf.reduce_sum(tf.reduce_sum(y*tf.math.log(y_pre)))
    return loss/tf.cast(tf.shape(y_pre)[0], tf.float64)
